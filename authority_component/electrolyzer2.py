...
# 定义冷却水出水温度范围
coolant_temperatures = range(20, 60, 5)  # 从20°C到55°C，步长为5°C
efficiencies = []

for temp in coolant_temperatures:
    print(f'Coolant temperature: {temp}°C')
    # 修改冷却水出水温度
    el_cw.set_attr(T=temp)
    ...
    # 获取电解槽的效率
    efficiencies.append(el.eta.val)

# 绘制电解槽效率随冷却水出水温度变化的图表
plt.figure(figsize=(10, 6))
plt.plot(coolant_temperatures, efficiencies, marker='o')
plt.title('Efficiency of Electrolyzer vs Cooling Water Outlet Temperature')
plt.xlabel('Cooling Water Outlet Temperature (°C)')
plt.ylabel('Electrolyzer Efficiency (%)')
plt.grid(True)
plt.show()
...
# 定义冷却水入水温度范围
coolant_temperatures = range(5, 20, 5)  # 从50°C到15°C，步长为5°C
efficiencies = []

for temp in coolant_temperatures:
    print(f'Coolant temperature: {temp}°C')
    # 修改冷却水入水温度
    cw_el.set_attr(T=temp)
...
# 绘制电解槽效率随冷却水入水温度变化的图表
plt.figure(figsize=(10, 6))
plt.plot(coolant_temperatures, efficiencies, marker='o')
plt.title('Efficiency of Electrolyzer vs Cooling Water inlet Temperature')
plt.xlabel('Cooling Water inlet Temperature (°C)')
plt.ylabel('Electrolyzer Efficiency (%)')
plt.grid(True)
plt.show()
...
# 定义el功率范围
el_power = np.arange(0.5, 2, 0.1) * P_design 
efficiencies = []

for temp in el_power :
    el_cmp.set_attr(v=None)
    print(f'el power: {temp * P_design} W')
    # 修改el 功率
    el.set_attr(P=temp)
...
# 绘制电解槽效率随冷却水出水温度变化的图表
plt.figure(figsize=(10, 6))
plt.plot(el_power, efficiencies, marker='o')
plt.title('Efficiency of Electrolyzer vs el Power')
plt.xlabel('el power (w)')
plt.ylabel('Electrolyzer Efficiency (%)')
plt.grid(True)
plt.show()
